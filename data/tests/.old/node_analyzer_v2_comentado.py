...
class TipoNodoAnalyzer:
    ...
    def _check_decorators_and_properties(
            self,
            line: str) -> Optional[TipoNodo]:
        if '@property' in line:
    ...
    def get_node_type(self, line: str) -> TipoNodo:
        ...
        if not line: # AGREGADA TOTALMENTE NUEVA
            return TipoNodo.WHITE_SPACE # AGREGADA TOTALMENTE NUEVA
        ...
        if '=' in line:
            index = line.index('=')
            pos1 = AnalizadorCadenas(
                ).encontrar_sin_comillas(line,
                '(',
                0,
                True
            )
            pos2 = AnalizadorCadenas(
                ).encontrar_sin_comillas(line,
                ')',
                0,
                True
