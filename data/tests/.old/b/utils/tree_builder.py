...
from analizador_cambios.utils.node_analyzer import TipoNodoAnalyzer
from analizador_cambios.utils.formateador_linea import FormateadorLinea
from analizador_cambios.config.node_types import PARENT_NODE_TYPES
...
class TreeBuilder:
    ...
    def build(self, lines: List[str]) -> Nodo:
        ...
        while i < len(lines):
            ...
            # Format long lines
            formatted_lines = FormateadorLinea.formatear_linea(line)
            ...
            if not self._analyze_docstring(line):
                ...
                if node_type == TipoNodo.WHITE_SPACE:
                    indent = previous_indent
                else:
                    indent = len(lines[i]) - len(lines[i].lstrip())
                    previous_indent = indent  # Update previous indent for non-white space
                ...
                line = lines[i]
            elif not self._is_line_complete(line.strip()):
                multilines = [i]
            ...
            new_node = Nodo(node_type, line.strip(), indent, numero_nodo)
            ...
            self.tree_2_lines[numero_nodo] = posicion_linea
            numero_nodo += 1
